...
export default function ProfilePage() {
  ...
  const handleAddPm = async () => {
    ...
    const isCard = newPmType === "debit_card" || newPmType === "credit_card";
    const isEmail = newPmType === "paypal" || newPmType === "wise";
    const last4 = isCard ? newPmDetails.cardNumber.slice(-4) : undefined;
    const [expiryMonth, expiryYear] = isCard ? newPmDetails.expiry.split("/").map(Number) : [undefined, undefined];
    const labels: Record<string, string> = { debit_card: "Debit Card", credit_card: "Credit Card", paypal: "PayPal", wise: "Wise", payoneer: "Payoneer", bank_swift: "Bank (SWIFT)" };
    const label = isCard ? `${labels[newPmType]} ending ${last4}` : isEmail ? `${labels[newPmType]} - ${newPmDetails.email}` : `${labels[newPmType]} - ${newPmDetails.accountId || "Account"}`;
    try {
      const res = await fetch("/api/community/payment-methods", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: newPmType, label, last4, expiryMonth, expiryYear: expiryYear && expiryYear < 100 ? 2000 + expiryYear : expiryYear, email: isEmail ? newPmDetails.email : undefined, accountId: newPmDetails.accountId || undefined, isDefault: paymentMethods.length === 0 }),
      });
      const data = await res.json();
