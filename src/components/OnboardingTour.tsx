...
export default function OnboardingTour() {
  ...
  if (targetRect) {
    ...
    const centerY = targetRect.top + targetRect.height / 2 + scrollY;

    switch (tooltipPosition) {
      case "bottom":
        tooltipLeft = Math.max(16, Math.min(window.innerWidth - tooltipWidth - 16, centerX - tooltipWidth / 2));
        tooltipTop = targetRect.bottom + scrollY + padding + 8;
        break;
      case "top":
        tooltipLeft = Math.max(16, Math.min(window.innerWidth - tooltipWidth - 16, centerX - tooltipWidth / 2));
        tooltipTop = targetRect.top + scrollY - padding - 8;
        break;
      case "left":
  ...
  // If tooltip would go off screen, adjust
  if (tooltipTop < 10) {
    tooltipTop = (targetRect?.bottom ?? 100) + 20;
    tooltipPosition = "bottom";
  }
  if (tooltipTop > window.innerHeight - 200) {
    tooltipTop = (targetRect?.top ?? window.innerHeight) - 220;
    tooltipPosition = "top";
  }
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl p-5 relative">
            {/* Close button */}
            <button
              onClick={handleSkip}
            ...
            {/* Title */}
            <h3 className="text-base font-semibold text-white mb-2 pr-6">
              {currentStep.title}
            </h3>
            ...
            <div className="flex items-center justify-between">
              <Button
                ...
                onClick={handleBack}
                disabled={isFirst}
                className="text-zinc-400 hover:text-white hover:bg-zinc-800 gap-1 disabled:opacity-30"
              >
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  size="sm"
                  onClick={handleSkip}
                  className="text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 gap-1"
                >
