...
export function VoiceCall({
  ...
  useEffect(() => {
    // Sanitize currentUserId for PeerJS
    ...
    newPeer.on('call', (call) => {
      ...
      // We don't answer yet, wait for user to click "Accept"
    });
  ...
  const startPeerCall = async () => {
    ...
    try {
      ...
      const call = peerInstanceRef.current.call(sanitizedTargetId, stream);
      callRef.current = call;
      handleCall(call);
    } catch (err) {
  ...
  const acceptCall = async () => {
    ...
    try {
      ...
      if (callRef.current) {
        ...
        // Wait for the call to arrive if it hasn't yet
